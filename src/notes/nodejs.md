...
- `process.nextTick()`
  ...
  用于安排在当前**操作**完成后立即执行回调，这对于您想要确保尽快执行回调，但仍在当前执行上下文之后的情况很有用。`process.nextTick()` 在技术上不是事件循环的一部分，nextTickQueue 将在当前**操作**完成后处理，无论事件循环的当前阶段如何。

  > [!NOTE]
  >
  > 这里的操作被定义为从底层 C/C++ 处理程序过渡，并处理需要执行的 JavaScript 代码。
